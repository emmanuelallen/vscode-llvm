...
import { CC1Command, ClangCommand, Command, CommandEnv, RustcCommand } from './clang';
...
export class Pipeline {
    ...
    public async run() {
        ...
        // Handle Rust compilation output
        if (this.command instanceof RustcCommand) {
            let c = this.command as RustcCommand;
            let output = c.getOutputPath();
            if (output) {
                let index = output.lastIndexOf('.');
                let basePath = output.substring(0, index);
                
                // Try to read the LLVM IR file generated by rustc
                try {
                    this.llvm = readFileSync(basePath + ".ll").toString();
                } catch (e) {
                    console.log("Could not read LLVM IR file:", e);
                }

                // Try to read the assembly file generated by rustc
                try {
                    this.output = readFileSync(basePath + ".s").toString();
                } catch (e) {
                    console.log("Could not read assembly file:", e);
                }
            }
        }
        ...
        try {
            ...
            // For Rust files, there's no .i file
            console.log("Could not read preprocessed file:", e);
        }
    }
